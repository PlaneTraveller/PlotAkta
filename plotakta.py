...
def setupParserOptions():
    parser = optparse.OptionParser(
        usage="Usage: %prog [options] filename", version="%prog 1.01 2020-02-24"
    )
    parser.add_option(
        "-c",
        "--conductance",
...
def mainloop(params):
    ...
    showOnlyFracs = params[
        "only_fracs"
    ]  # only show region in between first and last fractions
    useElutionMark = params[
        "only_elution"
    ]  # this is for when you also fractionated flow-through but only want to plot fractionated eluted protein
    ...
    if not aktafile_exists:
        ...
        print(
            "\nERROR: "
            + aktafile
            + " does not exist.Run plotakta.py -h to get the help text."
        )
    ...
    with open(aktafile, newline="") as items:
    ...
    plt.xlabel("Volume (mL)", fontsize=20, fontname=font)

    # ====== CHANGED HERE
    # plt.ylabel("Absorbance (mAU)", fontsize=20, fontname=font, color="darkblue")
    plt.ylabel("Absorbance 280nm (mAU)", fontsize=20, fontname=font, color="darkblue")
    plt.xticks(fontsize=20, fontname=font)
    plt.yticks(fontsize=20, fontname=font)
            ...
            if doDig1Label:
                ...
                for dx, df, dfx in zip(dig1lab_x, dig1lab_frac, dig1lab_x_frac):
                    ...
                    if dx < uv_x[-1]:
                        ...
                        ax2.plot(
                            [dx, dx], [0, 1], "--", color="k", alpha=0.6, linewidth=1
                        )
    ...
    if showUVmax:
        ...
        ax.plot(
            [maxvol, maxvol], [ymin, np.amax(uv_trace)], color="orange", linewidth=1.5
        )
        ...
        for fx, ff, fxl in zip(frac_x, fracs, frac_x_loc):
            ...
            if useElutionMark:
                ...
                if fx > log_x[log_text.index("Elution")]:
                    ...
                    ax.plot(
                        [fx, fx], [ymin, ymax], "--", color="k", alpha=0.6, linewidth=1
                    )
    ...
    plotName = os.path.basename(aktafile[:-4])
    plt.title(plotName, fontsize=30, fontname=titleFont)
